...
function normalizeOutputData(rawData) {
  ...
  const hierarchy = source.topic_hierarchy || source.topics || []
  const tags = (source.tags || []).map((tag) => {
    if (typeof tag === 'string') {
      return { name: tag, summary: '', start: null, end: null, sentences: [] }
    }
    return {
      ...tag,
      name: tag?.name || 'Unnamed cluster',
      sentences: Array.isArray(tag?.sentences) ? tag.sentences : [],
    }
  })

  return {
    ...source,
    topic_hierarchy: Array.isArray(hierarchy) ? hierarchy : [],
    tags,
  }
}
  ...
  tags: [
    {
      ...
      title_recommendations: [{ title: 'Why India’s Economy Keeps Feeling So Heavy', start_time: 0, end_time: 150 }],
    },
  ],
}
...
export function LegacyDashboard() {
  const [data, setData] = useState(() => {
    ...
    if (!saved) return normalizeOutputData(sampleData)

    try {
      return normalizeOutputData(JSON.parse(saved))
    } catch {
      return normalizeOutputData(sampleData)
    }
  })
...
function collectLeafNodesForTag(nodes, tagName, tagSentences) {
  const sentenceKeys = new Set(
    (tagSentences || []).map((sentence) => `${sentence.start}|${sentence.text}`),
  )
  ...
  function walk(items) {
    return (items || []).flatMap((node) => {
      ...
      const hasTag = (node.tags || []).some((tag) => tag.toLowerCase() === tagName.toLowerCase())
      const sharesSentence = (node.sentences || []).some((sentence) =>
        sentenceKeys.has(`${sentence.start}|${sentence.text}`),
      )
      return hasTag || sharesSentence ? [node] : []
    })
  }
...
function App() {
  ...
  const selectedTagNodes = useMemo(
    () => selectedTag
      ? collectLeafNodesForTag(data?.topic_hierarchy || [], selectedTag.name, selectedTag.sentences)
      : [],
    [data, selectedTag],
  )
  const selectedTagTitleCount = selectedTagNodes.reduce(
    (total, node) => total + (node.title_recommendations?.length || node.titles?.length || 0),
    0,
  )
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      const parsed = normalizeOutputData(JSON.parse(await file.text()))
      setData(parsed)
      setView('hierarchy')
                ...
                {view === 'tags' && filteredTags.map((tag) => (
                  ...
                  >
                    <span className="tag-symbol">#</span>
                          <span className="list-item-copy"><strong>{tag.name}</strong><small>{tag.sentences?.length || 0} sentences · {collectLeafNodesForTag(data?.topic_hierarchy || [], tag.name, tag.sentences).length} nodes</small></span>
                    <span className="list-chevron">›</span>
                  </button>
                ))}
            ...
            <section className="detail-panel">
              ...
              {view === 'tags' && selectedTag && (
                <>
                  ...
                  <div className="detail-rule" />
                  <div className="transcript-heading"><span>✦</span><h3>Titles from matching nodes</h3><b>{selectedTagTitleCount}</b></div>
                  <div className="tag-node-list">
                    {selectedTagNodes.map((node) => {
                      const titles = node.title_recommendations || node.titles || []
                      return (
                        <article key={node.node_id || node.name} className="tag-node-card">
                          <div className="tag-node-heading">
                            <div><span className="detail-label">Leaf node</span><h3>{node.name}</h3></div>
                            <span className="node-level">L{node.level || '—'}</span>
                          </div>
                          <p>{node.summary || 'No summary available for this node.'}</p>
                          {titles.length > 0 ? (
                            <div className="title-list">{titles.map((item, index) => <article key={`${item.title}-${index}`} className="title-item"><span>{String(index + 1).padStart(2, '0')}</span><div><strong>{item.title}</strong><small>{formatTime(item.start_time ?? item.start)} — {formatTime(item.end_time ?? item.end)}</small></div></article>)}</div>
                          ) : <small className="muted-note">No title recommendations for this node.</small>}
                        </article>
                      )
                    })}
                  </div>
                  {!selectedTagNodes.length && <div className="empty-state">No hierarchy nodes matched this tag.</div>}
